...
# load abundance data
dat <- read.csv(paste0(path, "/data/celldat_input.csv"))
...
saveRDS(lsamp, "detection/models/nf1.2.1nb_posterior.rds")
...
saveRDS(out, "detection/models/nf1.2.1nb_pars.rds")

# ========= 
saveRDS(lapply(lsamp, function(x) { return(x |> as.data.frame(x) |> 
                                     select(contains("mu")) ) }) |> bind_rows(), 
        "detection/models/nf1.2.1nb_mu.rds")

saveRDS(lapply(lsamp, function(x) { return(x |> as.data.frame(x) |> 
                                             select(contains("NTP")) ) }) |> bind_rows(), 
        "detection/models/nf1.2.1nb_NTP.rds")
